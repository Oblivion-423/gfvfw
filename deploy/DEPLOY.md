...
- **两处都能看到底图**（v2.9 起）：`/theater/{id}`（战役管理详情页）的
  「战区地图」面板与 `/theater/{id}/map`（战场态势图）。两处用**同一套**
  挑选规则与同一段缺图说明；详情页只放**一张纯底图**，切到哪张会带进态势图。
  ⚠️ 面板里的 `<img>` **刻意不带** `loading="lazy"` 与 `decoding="async"` ——
  实测这两个属性会让底图先画成**空框**（`decoding="async"` 尤其隐蔽：整页
  一个亮像素都没有）。别把它们当"漏掉的优化"加回去。
...
### 删成员 / 解绑账号：**访问权必须跟着一起撤掉**

这三个操作里唯一有安全含义的就是这两个，所以单独说清：

| 你要的效果 | 用哪个 | 账号会怎样 | 名册记录会怎样 |
|---|---|---|---|
| 这人**还在队里**，只是不该再用这个账号登录 | **解绑该账号** | 降为**游客**（能登录，看得到列表与汇总，**没有任何队内权限**） | **保留**（角色也保留） |
| 这人**离队了** | **作废此成员** | 一并**停用**（不能登录） | 标作废，从名册消失（历史架次与 Logbook 全保留） |
| 只是**绑错了账号**（A 的账号绑到 B 上了） | **解绑** → 再用 `/enroll` 绑到正确的成员 | 解绑后是游客；重绑后**照常继承该成员的角色** | 保留 |

> ⚠️ 曾经有个真实的口子：**作废成员只标了名册行，账号照样能登录、照样是队员**
...
* 不能作废／解绑**最后一个能登录的 owner** —— 那之后界面上再没有人能授权，
  只能上服务器用 `gfvfw.cli grant-role` 救。真被锁了就这样救：

```bash
sudo -u gfvfw -H bash -c 'cd /srv/gfvfw; set -a; . /etc/gfvfw/env; set +a; \
  .venv/bin/python -m gfvfw.cli grant-role --username <你的登录名> --role owner'
```
...
3. **作废战役不会自动归回任务。** 作废时任务被移出为「未归属」，而这个动作
   没有记录它们原本属于谁（事后分不清哪些本来就未归属）—— 所以恢复战役后
   请到详情页用「归入任务」重新归入。宁可让你多点一次，也不凭猜测挪数据。
4. **软删除一定配恢复入口。** 否则界面上写着"可恢复"而实际没有路径，
   那是在说假话。成员与战役的恢复都在列表页的「显示已作废」里。
...
* **想直接开一个队员账号（一步到位）**：把 `https://gfvfw.top/enroll` 发给本人，
  他自己建号 —— **不需要登录、不需要任何权限、没有审批**，建完立刻是队员。
  两种用法：**给名册里已有、但还没有账号的队员补一个账号**（下拉里只列这类人），
  或者**当场新建一个队员**（同时建名册行 + 账号 + 队员角色）。
  页面不提供"选角色" —— 要授权请用入队审批页或 `grant-role`。
  页面**不在任何导航里** —— 只能靠这个地址进入。
  > ⚠️⚠️ **公开 ⇒ 链接本身就是凭证。**
  > 拿到这个地址的任何人都能给自己开一个立刻生效的队员账号（能看队内全部
  > 内容、能提交数据）。「不在导航里」**不是**安全措施 —— 地址固定在源码里，
  > 读过代码的人都知道。所以：**发给谁、发到哪个群，自己拿捏。**
  >
  > 要收回公开（恢复为"需要 `application.review` 权限"），在
  > `/etc/gfvfw/env` 里加一行并重启，**不用改代码**：
  >
  > ```
  > GFVFW_ENROLL_OPEN=false
  > ```
  >
  > 另外有按来源 IP 的每日额度（默认 20 个，`GFVFW_MAX_ENROLL_PER_IP_PER_DAY`）
  > 挡批量刷号；它不改变"谁都能开号"，只挡"一个来源无限刷"。
* **登录名允许与呼号相同**（联队用呼号当登录名）。两个名字各自唯一即可，
  互相不冲突；导航栏会始终标出身份（游客 / 队员），所以「Viper 游客」与
  「Viper 队员」不会混。**呼号自身仍然唯一** —— 名册重名会让 ACMI 归并认错人。
